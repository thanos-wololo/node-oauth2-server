...
AuthorizeHandler.prototype.handle = function(request, response, expReqCtx) {
  ...
  const fns = [
    ...
    this.getClient(request, expReqCtx),
    this.getUser(request, response, expReqCtx)
  ];
    ...
    .spread(function(expiresAt, client, user) {
      ...
      return Promise.bind(this)
        .then(function() {
          ...
          return this.validateScope(user, client, requestedScope, expReqCtx);
        })
        .then(function(validScope) {
          scope = validScope;

          return this.generateAuthorizationCode(client, user, scope, expReqCtx);
        })
        .then(function(authorizationCode) {
          state = this.getState(request);
          ResponseType = this.getResponseType(request);

          return this.saveAuthorizationCode(authorizationCode, expiresAt, scope, client, uri, user, expReqCtx);
        })
        .then(function(code) {
...
AuthorizeHandler.prototype.generateAuthorizationCode = function(client, user, scope, expReqCtx) {
  if (this.model.generateAuthorizationCode) {
    return promisify(this.model.generateAuthorizationCode, 4).call(this.model, client, user, scope, expReqCtx);
  }
...
AuthorizeHandler.prototype.getClient = function(request, expReqCtx) {
  ...
  return promisify(this.model.getClient, 3).call(this.model, clientId, null, expReqCtx)
 ...
 */
AuthorizeHandler.prototype.validateScope = function(user, client, scope, expReqCtx) {
  if (this.model.validateScope) {
    return promisify(this.model.validateScope, 4).call(this.model, user, client, scope, expReqCtx)
      .then(function (scope) {
        if (!scope) {
...
AuthorizeHandler.prototype.getUser = function(request, response, expReqCtx) {
  if (this.authenticateHandler instanceof AuthenticateHandler) {
    return this.authenticateHandler.handle(request, response, expReqCtx).get('user');
  }
  return promisify(this.authenticateHandler.handle, 3)(request, response, expReqCtx).then(function(user) {
    if (!user) {
...
AuthorizeHandler.prototype.saveAuthorizationCode = function(authorizationCode, expiresAt, scope, client, redirectUri, user, expReqCtx) {
  const code = {
  ...
  return promisify(this.model.saveAuthorizationCode, 4).call(this.model, code, client, user, expReqCtx);
};
