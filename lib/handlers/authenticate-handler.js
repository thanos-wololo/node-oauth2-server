...
AuthenticateHandler.prototype.handle = function(request, response, expReqCtx) {
  ...
  return Promise.bind(this)
    ...
    .then(function(token) {
      return this.getAccessToken(token, expReqCtx);
    })
    .tap(function(token) {
    ...
    .tap(function(token) {
      ...
      return this.verifyScope(token, expReqCtx);
    })
    .tap(function(token) {
...
AuthenticateHandler.prototype.getAccessToken = function(token, expReqCtx) {
  return promisify(this.model.getAccessToken, 2).call(this.model, token, expReqCtx)
...
AuthenticateHandler.prototype.verifyScope = function(accessToken, expReqCtx) {
  return promisify(this.model.verifyScope, 3).call(this.model, accessToken, this.scope, expReqCtx)
    .then(function(scope) {
      if (!scope) {
