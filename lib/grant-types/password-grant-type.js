...
PasswordGrantType.prototype.handle = function(request, client, expReqCtx) {
  if (!request) {
  ...
  return Promise.bind(this)
    .then(function() {
      return this.getUser(request, expReqCtx);
    })
    .then(function(user) {
...
PasswordGrantType.prototype.getUser = function(request, expReqCtx) {
  ...
  return promisify(this.model.getUser, 3).call(this.model, request.body.username, request.body.password, expReqCtx)
    .then(function(user) {
      if (!user) {
...
PasswordGrantType.prototype.saveToken = function(user, client, scope, expReqCtx) {
  const fns = [
