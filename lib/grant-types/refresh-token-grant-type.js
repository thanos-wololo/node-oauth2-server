...
RefreshTokenGrantType.prototype.handle = function(request, client, expReqCtx) {
  if (!request) {
  ...
  return Promise.bind(this)
    .then(function() {
      return this.getRefreshToken(request, client, expReqCtx);
    })
    .tap(function(token) {
      return this.revokeToken(token, expReqCtx);
    })
    .then(function(token) {
      return this.saveToken(token.user, client, token.scope, expReqCtx);
    });
};
...
RefreshTokenGrantType.prototype.getRefreshToken = function(request, client, expReqCtx) {
  ...
  return promisify(this.model.getRefreshToken, 2).call(this.model, request.body.refresh_token, expReqCtx)
    .then(function(token) {
      if (!token) {
...
RefreshTokenGrantType.prototype.revokeToken = function(token, expReqCtx) {
  if (this.alwaysIssueNewRefreshToken === false) {
    return Promise.resolve(token);
  }

  return promisify(this.model.revokeToken, 2).call(this.model, token, expReqCtx)
...
RefreshTokenGrantType.prototype.saveToken = function(user, client, scope, expReqCtx) {
  const fns = [
