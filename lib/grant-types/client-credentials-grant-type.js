...
ClientCredentialsGrantType.prototype.handle = function(request, client, expReqCtx) {
  if (!request) {
  ...
  return Promise.bind(this)
    .then(function() {
      return this.getUserFromClient(client, expReqCtx);
    })
    .then(function(user) {
      return this.saveToken(user, client, scope, expReqCtx);
    });
};
...
ClientCredentialsGrantType.prototype.getUserFromClient = function(client, expReqCtx) {
  return promisify(this.model.getUserFromClient, 2).call(this.model, client, expReqCtx)
    .then(function(user) {
      if (!user) {
...
ClientCredentialsGrantType.prototype.saveToken = function(user, client, scope, expReqCtx) {
  const fns = [
