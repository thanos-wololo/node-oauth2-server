...
AuthorizationCodeGrantType.prototype.handle = function(request, client, expReqCtx) {
  if (!request) {
  ...
  return Promise.bind(this)
    .then(function() {
      return this.getAuthorizationCode(request, client, expReqCtx);
    })
    .tap(function(code) {
      return this.validateRedirectUri(request, code);
    })
    .tap(function(code) {
      return this.revokeAuthorizationCode(code, expReqCtx);
    })
    .then(function(code) {
      return this.saveToken(code.user, client, code.authorizationCode, code.scope, expReqCtx);
    });
};
...
AuthorizationCodeGrantType.prototype.getAuthorizationCode = function(request, client, expReqCtx) {
  ...
  return promisify(this.model.getAuthorizationCode, 2).call(this.model, request.body.code, expReqCtx)
    .then(function(code) {
      if (!code) {
...
AuthorizationCodeGrantType.prototype.revokeAuthorizationCode = function(code, expReqCtx) {
  return promisify(this.model.revokeAuthorizationCode, 2).call(this.model, code, expReqCtx)
...
AuthorizationCodeGrantType.prototype.saveToken = function(user, client, authorizationCode, scope, expReqCtx) {
  const fns = [
  ...
  return Promise.all(fns)
    ...
    .spread(function(scope, accessToken, refreshToken, accessTokenExpiresAt, refreshTokenExpiresAt) {
      ...
      return promisify(this.model.saveToken, 4).call(this.model, token, client, user, expReqCtx);
    });
};
